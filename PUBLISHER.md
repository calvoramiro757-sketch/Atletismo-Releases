...
El job `publish` es separado, solo se ejecuta tras validación satisfactoria y tiene `contents:write` bajo `production-release`. Descarga el paquete validado del mismo workflow run. El paso que vuelve a procesar archivos, metadata candidata y APK no recibe `GH_TOKEN` ni otro token en su entorno; el checkout tiene `persist-credentials: false`. En un paso posterior, con `GH_TOKEN` limitado a ese paso, consulta Stable pública y comprueba la ausencia de release y tag inmediatamente antes de crear un draft. Crea un tag nuevo de forma atómica contra el SHA confiable del publicador; si alguien lo creó mientras tanto, aborta en vez de reutilizarlo. Luego crea el draft con `--verify-tag`. Los dos jobs ejecutan la lógica del commit confiable del publicador en `main`, nunca scripts de `SOURCE_SHA`. Si la protección de reviewers del environment no se aplica realmente a esta cuenta/plan, no se debe usar `dry_run=false` hasta que exista una puerta de aprobación efectiva.

El blob confiable de `.github/workflows/build-release-apk.yml` en el repositorio fuente es `28e1851e0afd279e876a315299907752665df46f`. Sus cuatro Actions externas están fijadas a commits completos (checkout v4.2.2, setup-java v4.9.1, upload-artifact v4.6.2 y android-emulator-runner v2.38.0). El blob anterior `256f0e17a5661841dad0ef9b4873402263d91e09` se rechaza; los artifacts históricos de V3.4.3 no son candidatos para una publicación futura.
